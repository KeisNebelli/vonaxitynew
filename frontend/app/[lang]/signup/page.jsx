    ...
    <div style={{ minHeight:'100vh',background:C.neutral,padding:24,display:'flex',alignItems:'center',justifyContent:'center' }}>
      <div style={{ background:'#fff',borderRadius:20,border:`1px solid ${C.border}`,padding:'32px 28px',maxWidth:500,width:'100%',boxShadow:'0 4px 24px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display:'flex',gap:8,marginBottom:28 }}>
          {[1,2,3].map(s => <div key={s} style={{ flex:1,height:4,borderRadius:2,background:step>=s?C.teal:C.border,transition:'background 0.2s' }} />)}
        </div>

        {step===1 && (
          <div>
            <h2 style={{ fontSize:20,fontWeight:700,color:C.neutralDark,marginBottom:4 }}>{t(lang,'signup.step1Title')}</h2>
            <p style={{ fontSize:13,color:C.neutralMid,marginBottom:20 }}>{t(lang,'signup.step1Subtitle')}</p>
            {[{name:'basic',price:'€30',visits:1},{name:'standard',price:'€50',visits:2},{name:'premium',price:'€120',visits:4}].map(p => (
              <div key={p.name} onClick={() => setForm({...form,plan:p.name})} style={{ borderRadius:12,padding:'14px 16px',marginBottom:10,cursor:'pointer',border:`2px solid ${form.plan===p.name?C.teal:C.border}`,background:form.plan===p.name?'#e0f2fe':'#fff',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                <div style={{ fontSize:14,fontWeight:600,color:C.neutralDark,textTransform:'capitalize' }}>{p.name} — {p.visits} {p.visits===1?t(lang,'pricing.visitMonth'):t(lang,'pricing.visitsMonth')}</div>
        ...
        {step===2 && (
          <div>
            <h2 style={{ fontSize:20,fontWeight:700,color:C.neutralDark,marginBottom:16 }}>{t(lang,'signup.step2Title')}</h2>
            <input style={inp} placeholder={t(lang,'signup.fullName')} value={form.name} onChange={e => setForm({...form,name:e.target.value})} />
            <input style={inp} type="email" placeholder={t(lang,'signup.email')} value={form.email} onChange={e => setForm({...form,email:e.target.value})} />
            <input style={inp} type="password" placeholder={t(lang,'signup.password')} value={form.password} onChange={e => setForm({...form,password:e.target.value})} />
            <input style={inp} type="tel" placeholder={t(lang,'signup.phone')} value={form.phone} onChange={e => setForm({...form,phone:e.target.value})} />
            ...
            <div style={{ display:'flex',gap:10,marginTop:4 }}>
              <button onClick={() => setStep(1)} style={{ flex:'0 0 auto',padding:'12px 20px',borderRadius:10,border:`2px solid ${C.border}`,background:'#fff',cursor:'pointer',fontWeight:600 }}>{t(lang,'signup.back')}</button>
              <button onClick={() => setStep(3)} disabled={!form.name||!form.email||!form.password} style={{ flex:1,background:C.teal,color:'#fff',border:'none',borderRadius:10,padding:'12px',fontSize:15,fontWeight:700,cursor:'pointer',opacity:(!form.name||!form.email||!form.password)?0.4:1 }}>{t(lang,'signup.continue')}</button>
            </div>
          </div>
        )}
        ...
        {step===3 && (
          <div>
            <h2 style={{ fontSize:20,fontWeight:700,color:C.neutralDark,marginBottom:16 }}>{t(lang,'signup.step3Title')}</h2>
            <input style={inp} placeholder={t(lang,'signup.theirName')} value={form.relativeName} onChange={e => setForm({...form,relativeName:e.target.value})} />
            ...
            <input style={inp} placeholder={t(lang,'signup.theirAddress')} value={form.relativeAddress} onChange={e => setForm({...form,relativeAddress:e.target.value})} />
            <input style={inp} type="tel" placeholder={t(lang,'signup.theirPhone')} value={form.relativePhone} onChange={e => setForm({...form,relativePhone:e.target.value})} />
            ...
            <div style={{ background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:8,padding:'10px 14px',fontSize:12,color:'#92400e',marginBottom:14 }}>
              ⚠️ {t(lang,'signup.emergencyNote')} <strong>127</strong>.
            </div>
            <div style={{ display:'flex',gap:10 }}>
              <button onClick={() => setStep(2)} style={{ flex:'0 0 auto',padding:'12px 20px',borderRadius:10,border:`2px solid ${C.border}`,background:'#fff',cursor:'pointer',fontWeight:600 }}>{t(lang,'signup.back')}</button>
        ...
        <div style={{ textAlign:'center',marginTop:16,fontSize:13,color:C.neutralMid }}>
          {t(lang,'signup.alreadyAccount')} <Link href={`/${lang}/login`} style={{ color:C.teal,fontWeight:600 }}>{t(lang,'signup.signIn')}</Link>
        </div>
      </div>
    </div>
  );
