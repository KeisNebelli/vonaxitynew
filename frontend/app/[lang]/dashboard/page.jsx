...
const MOCK = {
  ...
  visits: [
    ...
    { id:'v2', serviceType:'Blood Pressure Check', scheduledAt:'2024-11-28T10:00:00Z', status:'COMPLETED', nurse:{ user:{ name:'Elona Berberi' } }, relative:{ name:'Fatmira Murati' }, bpSystolic:128, bpDiastolic:82, glucose:5.4, nurseNotes:'Patient in good spirits. BP slightly elevated.' },
  ],
};

const NAV = [
  { id:'overview', label:'Overview', icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg> },
  { id:'visits', label:'Visits', icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
  { id:'subscription', label:'Plan', icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg> },
...
function Sidebar({ user, active, setActive, lang, onLogout }) {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <div className="dash-sidebar" style={{ width:220, background:C.dark, display:'flex', flexDirection:'column', minHeight:'100vh', position:'sticky', top:0, flexShrink:0 }}>
      ...
      <div className="dash-tabbar" style={{ display:'none', position:'fixed', bottom:0, left:0, right:0, background:C.dark, borderTop:'1px solid rgba(255,255,255,0.1)', zIndex:100, padding:'8px 0 env(safe-area-inset-bottom)' }}>
        {NAV.map(item => (
          <button key={item.id} onClick={() => setActive(item.id)} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4, padding:'6px 4px', border:'none', background:'transparent', color:active===item.id?'#93C5FD':'rgba(255,255,255,0.4)', cursor:'pointer', fontSize:10, fontWeight:active===item.id?600:400 }}>
            {item.icon}<span>{item.label}</span>
          </button>
        ))}
      </div>
...
function Overview({ user, visits, relative }) {
  ...
  const subscription = user?.subscription || MOCK.subscription;

  return (
    <div>
      {nextVisit && (
        <div style={{ background:C.primaryLight, borderRadius:14, border:`1px solid rgba(37,99,235,0.15)`, padding:'18px 22px', marginBottom:24, display:'flex', gap:14, alignItems:'center' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))', gap:12, marginBottom:24 }}>
        {[
          ['Plan', (subscription?.plan||'standard').charAt(0).toUpperCase()+(subscription?.plan||'standard').slice(1), C.primary],
          ['Visits used', `${subscription?.visitsUsed||0} / ${subscription?.visitsPerMonth||2}`, C.textPrimary],
      ...
      {relative && (
        <div style={{ background:C.bgWhite, borderRadius:14, border:`1px solid ${C.border}`, padding:24 }}>
          ...
          {[['Name',relative.name],['City',relative.city],['Address',relative.address],['Nurse',nextVisit?.nurse?.user?.name||'Being assigned']].map(([k,v]) => (
...
function Visits({ visits }) {
  return (
    <div>
      {visits.map(v => (
        <div key={v.id} style={{ background:C.bgWhite, borderRadius:14, border:`1px solid ${C.border}`, padding:'20px', marginBottom:12 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:v.status==='COMPLETED'?12:0, gap:10 }}>
            <div>
              <div style={{ fontSize:15, fontWeight:600, color:C.textPrimary, marginBottom:4 }}>{v.serviceType}</div>
              <div style={{ fontSize:13, color:C.textTertiary }}>{new Date(v.scheduledAt).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})} · {v.nurse?.user?.name||'Nurse TBC'}</div>
            </div>
            <span style={{ fontSize:12, fontWeight:600, padding:'4px 12px', borderRadius:99, background:v.status==='COMPLETED'?C.secondaryLight:C.primaryLight, color:v.status==='COMPLETED'?C.secondary:C.primary, flexShrink:0 }}>{v.status}</span>
          </div>
          {v.status==='COMPLETED' && (
            <div style={{ background:C.bg, borderRadius:10, padding:'12px 16px', fontSize:13 }}>
              <div style={{ display:'flex', gap:16, marginBottom:v.nurseNotes?6:0, flexWrap:'wrap' }}>
...
export default function Dashboard({ params }) {
  ...
  const user = realUser ? { name:realUser.name, email:realUser.email, phone:realUser.phone||'', country:realUser.country||'', city:realUser.city||'', subscription:realUser.subscription } : MOCK.user;
  ...
  return (
    <>
      <style>{`
        ...
        .dash-tabbar { display: none !important; }
        .dash-main { padding: 28px !important; }
        @media (max-width: 768px) {
          .dash-sidebar { display: none !important; }
          .dash-tabbar { display: flex !important; }
          .dash-main { padding: 16px 16px 90px !important; }
      ...
      <div style={{ display:'flex', minHeight:'100vh', fontFamily:"'Inter',system-ui,sans-serif", background:C.bg }}>
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
          {/* Mobile header */}
          <div className="dash-header" style={{ padding:'0 28px', height:60, borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'space-between', background:C.bgWhite, flexShrink:0 }}>
            <div>
              <div style={{ fontSize:16, fontWeight:600, color:C.textPrimary }}>{titles[active]}</div>
              <div className="dash-mobile-name" style={{ display:'none', fontSize:12, color:C.textTertiary }}>{user?.name}</div>
            </div>
          ...
          <main className="dash-main" style={{ flex:1, padding:28, overflowY:'auto', maxWidth:760, width:'100%' }}>
            {active==='overview' && <Overview user={user} visits={visits} relative={relative} />}
            ...
            {active==='subscription' && (
              <div style={{ background:C.bgWhite, borderRadius:14, border:`1px solid ${C.border}`, padding:28 }}>
                ...
                <div style={{ fontSize:15, color:C.primary, fontWeight:500, marginBottom:8 }}>{subscription?.visitsPerMonth||2} visits/month</div>
                <div style={{ fontSize:13, color:C.textTertiary, marginBottom:24 }}>Status: <strong style={{ color:subscription?.status==='ACTIVE'?C.secondary:C.warning }}>{subscription?.status||'TRIAL'}</strong></div>
                <button style={{ background:C.primary, color:'#fff', border:'none', borderRadius:10, padding:'12px 24px', fontSize:14, fontWeight:600, cursor:'pointer', width:'100%' }}>Upgrade to Premium</button>
              </div>
            )}
