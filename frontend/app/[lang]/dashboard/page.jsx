...
function fmtDate(isoStr, lang, opts = {}) {
  if (!isoStr) return '';
  ...
  const dow = d.getDay();
  const time = opts.hour ? ` · ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}` : '';
  if (opts.weekday === 'long') {
    if (opts.year) return `${SQ_DAYS[dow]}, ${day} ${SQ_MONTHS[month]} ${year}${time}`;
    return `${SQ_DAYS[dow]}, ${day} ${SQ_MONTHS[month]}${time}`;
  }
  if (opts.weekday === 'short') {
    if (opts.month === 'short') return `${SQ_DAYS_SHORT[dow]}, ${day} ${SQ_MONTHS_SHORT[month]} ${year}${time}`;
    return `${SQ_DAYS_SHORT[dow]}, ${day} ${SQ_MONTHS[month]} ${year}${time}`;
  }
  if (opts.month === 'short') return `${day} ${SQ_MONTHS_SHORT[month]} ${year}${time}`;
  return `${day} ${SQ_MONTHS[month]} ${year}${time}`;
}
          ...
          <div style={{ position:'relative', zIndex:1, display:'flex', alignItems:'center', gap:14, flexWrap:'wrap' }}>
            ...
            <div style={{ flex:1, minWidth:150 }}>
              ...
              <div style={{ fontSize:17, fontWeight:800, marginBottom:3, letterSpacing:'-0.3px' }}>
                {fmtDate(next.scheduledAt, lang, {weekday:'long',day:'numeric',month:'long'})} · {new Date(next.scheduledAt).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}
              </div>
        ...
        return (
          <div style={{ background:C.bgWhite, borderRadius:16, border:`1px solid ${C.border}`, overflow:'hidden', boxShadow:SSM }}>
            ...
            {recent.map((v, i) => {
              ...
              const d = new Date(v.scheduledAt);
              const dateStr2 = fmtDate(v.scheduledAt, lang, { day:'numeric', month:'short', year:'numeric' });
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr' }}>
            ...
            <div style={{ padding:'14px 20px', borderRight:`1px solid ${C.borderSubtle}` }}>
              ...
              <div style={{ fontSize:13, fontWeight:600, color: last ? C.textPrimary : C.textTertiary }}>
                {last ? fmtDate(last.scheduledAt, lang, {day:'numeric',month:'short',year:'numeric'}) : '—'}
              </div>
            </div>
            {/* Upcoming */}
...
function BookingConfirmation({ visit, onClose, onViewApplicants, lang='en' }) {
  ...
  const dateStr = dt ? fmtDate(visit.scheduledAt, lang, {weekday:'long',day:'numeric',month:'long',year:'numeric'}) : '';
  ...
  return (
    <div>
      ...
      <div style={{ background:C.bgWhite, borderRadius:14, border:`1px solid ${C.border}`, padding:'16px 20px', marginBottom:20, boxShadow:SSM }}>
        ...
        <div style={{ fontSize:12, color:C.textTertiary, marginTop:3 }}>{visitInfo?.scheduledAt ? fmtDate(visitInfo.scheduledAt, lang, {weekday:'long',day:'numeric',month:'long',year:'numeric'}) : ''}</div>
      </div>
...
function DeleteConfirm({ visit, onConfirm, onCancel, lang='en' }) {
  ...
  const dateStr = fmtDate(visit.scheduledAt, lang, {day:'numeric',month:'long',year:'numeric'});
  ...
  return (
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      {filteredVisits.map(v=>{
        ...
        const dateStr = fmtDate(v.scheduledAt, lang, {weekday:'short',day:'numeric',month:'short',year:'numeric'});
            ...
            <div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
                <span style={{ fontSize:12, color:'rgba(255,255,255,0.75)', display:'flex', alignItems:'center', gap:4 }}>
                  ...
                  {fmtDate(visit.scheduledAt, lang, {weekday:'short',day:'numeric',month:'long',year:'numeric'})}
                </span>
                <span style={{ fontSize:12, color:'rgba(255,255,255,0.75)', display:'flex', alignItems:'center', gap:4 }}>
