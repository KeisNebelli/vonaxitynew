      ...
      {open && (
        <div style={{ position:'absolute', top:42, right:0, width:320, background:C.bgWhite, borderRadius:14, boxShadow:'0 8px 30px rgba(15,23,42,0.12)', border:`1px solid ${C.border}`, zIndex:9999, overflow:'hidden' }}>
          ...
          <div style={{ padding:'14px 16px 10px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ fontSize:13, fontWeight:700, color:C.textPrimary }}>{lang==='sq' ? 'Njoftimet' : 'Notifications'} {unread > 0 && <span style={{ fontSize:11, fontWeight:600, padding:'2px 7px', borderRadius:99, background:C.primaryLight, color:C.primary, marginLeft:6 }}>{unread}</span>}</div>
            {unread > 0 && <button onClick={markAll} style={{ fontSize:11, fontWeight:600, color:C.primary, background:'none', border:'none', cursor:'pointer', padding:0 }}>{lang==='sq' ? 'Shëno të gjitha' : 'Mark all read'}</button>}
          </div>
            ...
            {notifs.length === 0 ? (
              ...
              >
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:13, fontWeight:n.read ? 500 : 700, color:C.textPrimary, marginBottom:2, lineHeight:1.3 }}>{n.title}</div>
                  <div style={{ fontSize:12, color:C.textSecondary, lineHeight:1.4, marginBottom:4 }}>{n.message}</div>
