  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: C.neutral, padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 20, border: `1px solid ${C.border}`, padding: '36px 32px', maxWidth: 420, width: '100%', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 700, color: C.neutralDark, marginBottom: 24 }}>Sign in</h1>

        <form onSubmit={handleLogin}>
          <label style={{ fontSize: 12, fontWeight: 600, color: C.neutralDark, display: 'block', marginBottom: 5 }}>Email</label>
          ...
          <label style={{ fontSize: 12, fontWeight: 600, color: C.neutralDark, display: 'block', marginBottom: 5 }}>Password</label>
          <input style={{ ...inp, marginBottom: 20 }} type="password" placeholder="Your password" required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
        ...
        <div style={{ marginTop: 20, fontSize: 13, color: C.neutralMid, textAlign: 'center' }}>
          No account? <Link href={`/${lang}/signup`} style={{ color: C.teal, fontWeight: 600 }}>Sign up</Link>
        </div>

        {/* Test accounts hint */}
        <div style={{ marginTop: 20, background: C.neutral, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: C.neutralMid, marginBottom: 8 }}>TEST ACCOUNTS</div>
          {[
            ['client@test.com', 'test123', 'Client'],
            ['nurse@test.com', 'test123', 'Nurse'],
        ...
        <div style={{ marginTop: 16, background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#991b1b', textAlign: 'center' }}>
          ⚠️ Medical emergency in Albania? Call <strong>127</strong>
        </div>
      </div>
    </div>
  );
