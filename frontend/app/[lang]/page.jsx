    ...
    try {
      ...
      const response = await fetch(`${BASE}/ai/chat`, {
        ...
        body: JSON.stringify({
          system: CONTEXT,
          messages:[
            ...messages.filter((m,idx)=>!(m.role==='assistant'&&idx===0)).map(m=>({ role:m.role, content:m.content })),
      ...
      setMessages(prev=>[...prev,{ role:'assistant', content:`Error: ${err.message}. Make sure ANTHROPIC_API_KEY is set in Railway.` }]);
