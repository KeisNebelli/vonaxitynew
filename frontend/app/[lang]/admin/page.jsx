...
// ── Nurses ────────────────────────────────────────────────────────────────────
function Nurses({ nurses, setNurses, onApprove, onSuspend }) {
  const [search, setSearch] = useState('');
        ...
        {pendingNurses.length===0 ? <div style={{ padding:'16px 20px', fontSize:13, color:C.textTertiary }}>No pending applications.</div> : pendingNurses.map(n => (
          <div key={n.id} style={{ padding:'14px 20px', borderBottom:`1px solid ${C.borderSubtle}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            ...
            <div style={{ display:'flex', gap:8 }}>
              ...
              <button onClick={()=>onSuspend(n.id)} style={{ fontSize:12, fontWeight:600, padding:'7px 14px', background:C.errorLight, color:C.error, border:'none', borderRadius:8, cursor:'pointer' }}>Reject</button>
            </div>
          </div>
        ))}
      </AlertGroup>
...
export default function AdminPage({ params }) {
  ...
  const loadData = async () => {
    try {
      ...
      if (nursesData?.nurses?.length > 0) setNurses(nursesData.nurses);
      if (usersData?.users?.length > 0) setClients(usersData.users);
      if (visitsData?.visits?.length > 0) setVisits(visitsData.visits);
      if (paymentsData?.payments?.length > 0) setPayments(paymentsData.payments);
    } catch (err) {
    ...
    <div style={{ display:'flex', minHeight:'100vh', fontFamily:"'Inter',system-ui,sans-serif", background:C.bg }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
        ...
        <main style={{ flex:1, padding:28, overflowY:'auto' }}>
          ...
          {active==='nurses' && <Nurses nurses={nurses} setNurses={setNurses} onApprove={handleApprove} onSuspend={handleSuspend} />}
          {active==='visits' && <Visits visits={visits} setVisits={setVisits} nurses={nurses} onAssign={handleAssign} />}
          {active==='alerts' && <Alerts visits={visits} nurses={nurses} setVisits={setVisits} setNurses={setNurses} onApprove={handleApprove} onSuspend={handleSuspend} onAssign={handleAssign} />}
