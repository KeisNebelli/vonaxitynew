...
const C = {
  teal: '#0e7490', sage: '#16a34a', border: '#e7e5e4',
  neutralMid: '#78716c', white: '#ffffff',
};
...
export default function Nav({ lang = 'en' }) {
  ...
  const links = [
    { label: lang === 'sq' ? 'Si Funksionon' : 'How It Works', href: `/${lang}/how-it-works` },
    { label: lang === 'sq' ? 'Shërbimet' : 'Services', href: `/${lang}/services` },
    { label: lang === 'sq' ? 'Çmimet' : 'Pricing', href: `/${lang}/pricing` },
    { label: lang === 'sq' ? 'Rreth Nesh' : 'About', href: `/${lang}/about` },
    { label: lang === 'sq' ? 'Kontakti' : 'Contact', href: `/${lang}/contact` },
  ];
    ...
    <nav style={{
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
        ...
        <div style={{ display: 'flex', gap: 28, alignItems: 'center' }}>
          {links.map((l) => (
            <Link key={l.href} href={l.href} style={{ fontSize: 14, color: C.neutralMid, textDecoration: 'none' }}>
              {l.label}
            </Link>
          ))}
        </div>
          ...
          <div style={{ display: 'flex', background: '#f8f7f4', borderRadius: 8, padding: 3, border: `1px solid ${C.border}` }}>
            {['en', 'sq'].map((l) => (
              <button key={l} onClick={() => switchLang(l)} style={{
                ...
                cursor: 'pointer', background: lang === l ? C.teal : 'transparent',
                color: lang === l ? '#fff' : C.neutralMid,
              }}>
                {l === 'en' ? '🇬🇧 EN' : '🇦🇱 SQ'}
              </button>
            ))}
