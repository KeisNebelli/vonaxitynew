...
const T = {
  en: {
    ...
    matchingNow: "We're matching you with a nurse now",
    homeVisit: 'Home Visit',
    findingNurse: 'Finding nurse…',
    searchingNurses: 'Searching nearby nurses…',
    matchedHours: 'Typically matched within a few hours',
    notifiedOnce: "You'll be notified once a nurse is assigned",
    date: 'Date', time: 'Time', location: 'Location', plan: 'Plan',
    /* MockupApplicants */
    nursesReady: '2 nurses ready for your visit',
    pickOrLet: 'Pick one or let us choose for you',
    recommended: 'RECOMMENDED',
    verified: 'Verified',
    exp: 'exp',
    selectConfirm: 'Select · Confirm visit →',
    viewProfile: 'View profile',
    letVonaxity: 'Or tap',
    letVonaxityBold: 'Let Vonaxity choose',
    letVonaxityEnd: 'for the best match',
    /* MockupNurseConfirmed */
    nurseConfirmed: 'Nurse confirmed!',
    arrivingTime: 'Arriving Mon May 5 · 10:00 AM',
    services: 'Services',
    smsSent: 'SMS + email reminder sent',
    /* MockupVisitTracking */
    nurseArrived: 'Nurse has arrived · Live',
    eta: 'ETA: Arrived 10:03 AM',
    onSite: 'On site',
    now: 'NOW',
    ...
    /* MockupHealthReport */
    visitCompleted: 'Visit completed · May 5',
  ...
  sq: {
    ...
    patientDetails: 'Të dhënat e pacientit',
    ...
    matchingNow: 'Po ju përputhim me një infermiere tani',
    homeVisit: 'Vizitë Shtëpie',
    findingNurse: 'Duke gjetur infermieren…',
    searchingNurses: 'Duke kërkuar infermierë afër…',
    matchedHours: 'Zakonisht përputhet brenda disa orësh',
    notifiedOnce: 'Do të njoftoheni sapo të caktohet një infermiere',
    date: 'Data', time: 'Ora', location: 'Vendndodhja', plan: 'Plani',
    /* MockupApplicants */
    nursesReady: '2 infermierë gati për vizitën tuaj',
    pickOrLet: "Zgjidhni njërin ose lërini ne t'ju zgjedhim",
    recommended: 'E REKOMANDUAR',
    verified: 'E verifikuar',
    exp: 'vit',
    selectConfirm: 'Zgjidh · Konfirmo vizitën →',
    viewProfile: 'Shiko profilin',
    letVonaxity: 'Ose shtypni',
    letVonaxityBold: 'Lëre Vonaxity të zgjedhë',
    letVonaxityEnd: 'për përputhjen më të mirë',
    /* MockupNurseConfirmed */
    nurseConfirmed: 'Infermierja u konfirmua!',
    arrivingTime: 'Mbërrin Hën 5 Maj · 10:00 PD',
    services: 'Shërbime',
    smsSent: 'Kujtesa me SMS + email u dërgua',
    /* MockupVisitTracking */
    nurseArrived: 'Infermierja mbërriti · Live',
    eta: 'Mbërriti: 10:03 PD',
    onSite: 'Në vend',
    now: 'TANI',
    ...
    /* MockupHealthReport */
    visitCompleted: 'Vizita u krye · 5 Maj',
    healthReport: 'Raporti Shëndetësor',
    normal: '✓ Normale',
    nurseNote: 'Shënim Infermiereje',
  ...
  return (
    <div style={{ background:'#F8FAFF', borderRadius:16, overflow:'hidden', border:'1px solid #E5E7EB', fontFamily:'Inter,system-ui,sans-serif', fontSize:12 }}>
      ...
      <div style={{ padding:'18px 16px' }}>
        <div style={{ fontSize:13, fontWeight:700, color:'#111827', marginBottom:12 }}>{tr(lang,'choosePlan')}</div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:7, marginBottom:14 }}>
          {displayPlans.map((p) => (
            <div key={p.name} style={{ border: p.featured ? '2px solid #7C3AED' : '1px solid #E5E7EB', borderRadius:10, padding:'9px 7px', textAlign:'center', background: p.featured ? '#F5F3FF' : '#fff', position:'relative' }}>
              {p.featured && <div style={{ position:'absolute', top:-8, left:'50%', transform:'translateX(-50%)', background:'#7C3AED', color:'#fff', fontSize:8, fontWeight:700, padding:'2px 7px', borderRadius:99, whiteSpace:'nowrap' }}>{tr(lang,'popular')}</div>}
              ...
              <div style={{ fontSize:9, color:'#9CA3AF', marginTop:1 }}>{tr(lang,'visitMo')(p.visits)}</div>
            </div>
          ))}
        </div>
        <div style={{ fontSize:10, fontWeight:700, color:'#6B7280', textTransform:'uppercase', letterSpacing:'0.8px', marginBottom:7 }}>{tr(lang,'patientDetails')}</div>
        {[[tr(lang,'fullName'),'Fatmira Murati'],[tr(lang,'city'),'Tirana'],[tr(lang,'servicesNeeded'),'Blood pressure, glucose check']].map(([label,val]) => (
      ...
      <div style={{ padding:'16px' }}>
        ...
        <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:12, padding:'13px', marginBottom:10 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:7, marginBottom:10 }}>
            {[[tr(lang,'date'),'Mon, May 5'],[tr(lang,'time'),'10:00 AM'],[tr(lang,'location'),'Tirana'],[tr(lang,'plan'),'Standard']].map(([k,v]) => (
        ...
        <div style={{ background:'#EFF6FF', borderRadius:8, padding:'8px 12px', fontSize:10, color:'#2563EB', fontWeight:600 }}>
          <span style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81 19.79 19.79 0 011 1.18 2 2 0 012.98 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L7.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 14.92z"/></svg>
            {tr(lang,'notifiedOnce')}
          </span>
        </div>
      </div>
    </div>
...
function MockupApplicants({ lang = 'en' }) {
  const nurses = [
    { name:'Elona Berberi', spec:'General Nursing', exp:'6', rating:'5.0', reviews:38, photo:'/nurse-elona.png', top:true  },
    { name:'Mirela Daka',   spec:'Blood Work',      exp:'4', rating:'4.9', reviews:21, photo:'/nurse-mirela.png', top:false },
  ];
  return (
    <div style={{ background:'#F8FAFF', borderRadius:16, overflow:'hidden', border:'1px solid #E5E7EB', fontFamily:'Inter,system-ui,sans-serif', fontSize:12 }}>
      <BrowserBar url="vonaxity.com/dashboard" />
        ...
        <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:12, padding:'13px' }}>
          <div style={{ display:'flex', gap:11, alignItems:'center', marginBottom:11 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:'#2563EB', fontWeight:500 }}>General Nursing · 6 {tr(lang,'exp')}</div>
            ...
            <div style={{ marginLeft:'auto', background:'#EFF6FF', border:'1px solid #BFDBFE', borderRadius:8, padding:'3px 8px', fontSize:10, fontWeight:700, color:'#2563EB' }}>{tr(lang,'verified')}</div>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:7 }}>
            {[[tr(lang,'date'),'Mon, May 5'],[tr(lang,'time'),'10:00 AM'],[tr(lang,'services'),'BP + Glucose'],[tr(lang,'location'),'Tirana, AL-01']].map(([k,v]) => (
...
function MockupVisitTracking({ lang = 'en' }) {
  const stepLabels = tr(lang, 'trackSteps');
  const steps = [
    { done:true  },
    ...
    <div style={{ background:'#F8FAFF', borderRadius:16, overflow:'hidden', border:'1px solid #E5E7EB', fontFamily:'Inter,system-ui,sans-serif', fontSize:12 }}>
      ...
      <div style={{ padding:'16px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:13 }}>
          ...
          <div style={{ fontSize:12, fontWeight:700, color:'#059669' }}>{tr(lang,'nurseArrived')}</div>
        </div>
        {/* Nurse strip */}
        <div style={{ display:'flex', gap:10, alignItems:'center', background:'#fff', border:'1px solid #E5E7EB', borderRadius:11, padding:'10px 12px', marginBottom:14 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:10, color:'#6B7280' }}>{tr(lang,'eta')}</div>
          </div>
          <div style={{ background:'#ECFDF5', borderRadius:7, padding:'4px 9px', fontSize:10, fontWeight:700, color:'#059669' }}>{tr(lang,'onSite')}</div>
        </div>
        {/* Timeline */}
          ...
          {steps.map((s, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:11, position:'relative' }}>
              ...
              <div style={{ fontSize:11, fontWeight: s.active ? 700 : s.done ? 600 : 400, color: s.active ? '#059669' : s.done ? '#374151' : '#9CA3AF' }}>
                {stepLabels[i]}
                {s.active && <span style={{ marginLeft:6, fontSize:9, background:'#ECFDF5', color:'#059669', padding:'1px 6px', borderRadius:99, fontWeight:700 }}>{tr(lang,'now')}</span>}
              </div>
            </div>
          ))}
        </div>
...
function MockupHealthReport({ lang = 'en' }) {
  const vitalLabels = tr(lang, 'vitals');
  const vitals = [
    ...
    <div style={{ background:'#F8FAFF', borderRadius:16, overflow:'hidden', border:'1px solid #E5E7EB', fontFamily:'Inter,system-ui,sans-serif', fontSize:12 }}>
      ...
      <div style={{ padding:'16px' }}>
        ...
        <div style={{ background:'linear-gradient(135deg,#EFF6FF,#F5F3FF)', border:'1px solid #C4B5FD', borderRadius:10, padding:'9px 13px', display:'flex', gap:10, alignItems:'center', marginBottom:13 }}>
          ...
          <div>
            <div style={{ fontSize:11, fontWeight:700, color:'#4C1D95' }}>{tr(lang,'visitCompleted')}</div>
            <div style={{ fontSize:10, color:'#7C3AED' }}>10:00–10:48 AM · Elona Berberi</div>
          </div>
        </div>
        {/* Vitals grid */}
        <div style={{ fontSize:11, fontWeight:700, color:'#111827', marginBottom:9 }}>{tr(lang,'healthReport')}</div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:7, marginBottom:11 }}>
          {vitals.map((r, idx) => (
            <div key={idx} style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:10, padding:'9px 10px' }}>
              <div style={{ fontSize:9, color:'#9CA3AF', marginBottom:3 }}>{vitalLabels[idx]}</div>
              ...
              <div style={{ fontSize:9, fontWeight:700, color:'#059669', marginTop:3 }}>{tr(lang,'normal')}</div>
            </div>
          ))}
        </div>
        {/* Nurse note */}
        <div style={{ background:'#FFFBEB', border:'1px solid #FDE68A', borderRadius:8, padding:'8px 10px', marginBottom:9 }}>
          <div style={{ fontSize:9, fontWeight:700, color:'#92400E', marginBottom:2, textTransform:'uppercase', letterSpacing:'0.5px' }}>{tr(lang,'nurseNote')}</div>
          <div style={{ fontSize:10, color:'#78350F', lineHeight:1.6 }}>{tr(lang,'nurseNoteText')}</div>
        </div>
...
/* ─────────────────────────────────────────────────────────
   STEP DEFINITIONS — language-aware
───────────────────────────────────────────────────────── */
const getSteps = (lang) => {
  const isAl = lang === 'sq';
  return [
    {
      num: '01',
      title: isAl ? 'Zgjidhni planin & rezervoni' : 'Pick a plan & book',
      color: '#7C3AED',
      colorLight: 'rgba(124,58,237,0.08)',
      badge: isAl ? 'Zgjat 2 minuta' : 'Takes 2 minutes',
      headline: isAl ? 'Zgjidhni planin, tregoni për të dashurin tuaj' : 'Pick a plan, tell us about your loved one',
      description: isAl
    ...
    {
      num: '02',
      title: isAl ? 'Puna u postua te infermierët' : 'Job posted to nurses',
      color: '#0EA5E9',
      colorLight: 'rgba(14,165,233,0.08)',
      badge: isAl ? 'Postim i menjëhershëm' : 'Instant posting',
      headline: isAl ? 'Rezervimi juaj shkon live te infermierët e verifikuar afër' : 'Your booking goes live to verified nurses nearby',
      description: isAl
    ...
    {
      num: '03',
      title: isAl ? 'Infermierët aplikojnë' : 'Nurses apply',
      color: '#2563EB',
      colorLight: 'rgba(37,99,235,0.08)',
      badge: isAl ? 'Ju zgjidhni ose ne përputhim' : 'You choose or we match',
      headline: isAl ? 'Shikoni aplikantët ose lërini ne të zgjedhim ndeshjen më të mirë' : 'Review applicants or let us pick the best match',
      description: isAl
        ? 'Mund të shfletoni profilet e infermierëve — vlerësimet, specialitetet, vitet e përvojës — dhe të zgjidhni atë që preferoni. Ose lërini ekipin tonë të zgjedhë infermieren me vlerësimet më të larta, të përputhur më mirë.'
    ...
    {
      num: '04',
      title: isAl ? 'Infermierja u konfirmua' : 'Nurse confirmed',
      color: '#059669',
      colorLight: 'rgba(5,150,105,0.08)',
      badge: isAl ? 'Brenda 24 orësh' : 'Within 24 hours',
      headline: isAl ? 'Infermierja juaj konfirmohet me orën dhe detajet e sakta' : 'Your nurse is confirmed with exact time and details',
      description: isAl
    ...
    {
      num: '05',
      title: isAl ? 'Infermierja ju viziton' : 'Nurse visits you',
      color: '#D97706',
      colorLight: 'rgba(217,119,6,0.08)',
      badge: isAl ? 'Tek dera juaj' : 'At your doorstep',
      headline: isAl ? 'Gjurmoni infermieren tuaj në kohë reale ndërsa mbërrin' : 'Track your nurse in real-time as they arrive',
      description: isAl
    ...
    {
      num: '06',
      title: isAl ? 'Raporti shëndetësor dërgohet' : 'Health report delivered',
      color: '#7C3AED',
      colorLight: 'rgba(124,58,237,0.08)',
      badge: isAl ? 'Të njëjtën ditë' : 'Same day',
      headline: isAl ? 'Raporti i plotë i vitaleve dhe shënimet e infermierës — direkt në emailin tuaj' : 'Full vitals report and nurse notes — straight to your inbox',
      description: isAl
...
export default function HowItWorksSection({ lang = 'en', tag, title, subtitle, steps: stepsProp, plans: plansProp }) {
  ...
  const goNext = useCallback(() => { if (active < STEPS.length - 1) goTo(active + 1, 'next'); }, [active, goTo, STEPS.length]);
    ...
    <>
      ...
      <section id="how-it-works" style={{ padding:'96px 24px', background:'#fff', scrollMarginTop:'76px', position:'relative', zIndex:1, overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:1140, margin:'0 auto' }}>
          ...
          {/* 6-card grid: fixed 3 columns on desktop */}
