...
export default function MapComponent({ patientLat, patientLng, patientName='Patient', patientAddress='', height='300px', apiKey=process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY||'', zoom=15 }) {

  const hasCoords = patientLat && patientLng;
  const query = hasCoords ? `${patientLat},${patientLng}` : encodeURIComponent(patientAddress || patientName);
  const mapsUrl = hasCoords
    ? `https://www.google.com/maps/search/?api=1&query=${patientLat},${patientLng}`
    : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(patientAddress || patientName)}`;
  ...
  if (!apiKey) {
    return (
      <a href={mapsUrl} target="_blank" rel="noopener noreferrer" style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height, background:C.primaryLight, borderRadius:12, border:`1px solid rgba(37,99,235,0.15)`, textDecoration:'none', gap:10, cursor:'pointer' }}>
        ...
        {patientAddress && <div style={{ fontSize:12, color:C.textSecondary, textAlign:'center', maxWidth:'80%' }}>{patientAddress}</div>}
  ...
  const embedUrl = hasCoords
    ? `https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=${patientLat},${patientLng}&zoom=${zoom}`
    : `https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=${encodeURIComponent(patientAddress || patientName)}&zoom=${zoom}`;
